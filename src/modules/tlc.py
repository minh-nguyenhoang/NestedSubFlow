...
class AvgPool2d(nn.Module):
    def __init__(self, kernel_size=None, base_size=256, auto_pad=True, fast_imp=False, train_size=(1,3,256,256)):
        super().__init__()
...
class LocalHalfInstanceNorm2d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1,
                 affine=False, track_running_stats=False):
        super().__init__()
        assert not track_running_stats
        self.affine = affine
        if self.affine:
            self.weight = nn.Parameter(torch.ones(num_features//2))
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.affine:
            input = input * self.weight.view(1,-1, 1, 1) + self.bias.view(1,-1, 1, 1)
        input = torch.cat([input, res], dim=1)
        return input
    

def replace_layers(model: nn.Module, base_size, fast_imp, **kwargs):
...
class Local_Base():
    def convert(self: nn.Module, *args, train_size, **kwargs):
