#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# This file is copied from https://github.com/giangbang/Pure-Pytorch-implementation-Deformable-Convolution/blob/master/deform_conv_torch/deform_conv.py
# I've modified the deformable convolution a little bit so the offset_conv does not need to be provided for each group. 
# This would probaly save up a huge number of parameters for separable convolution. (By a factor of num_groups)
# For the normal implementation you can still make the offset convolution a separable convolution, but I personally don't like this as it require for the same number of groups.
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Still didn't fully understand how the gradient of the offset are propagated, but the author have clarified and ensured the rightness of the function.
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def separable_deform_conv2d(
    ...
    """
    This improves the normal deformable convolution by allowing the disentanglement 
    between groups and offset_groups.\n
    This allows the parameter efficiency of the offset convolution and allow the chance of 
    sharing offset between separable convolutions.
    """
    ...
    offset_groups = offset.shape[1] // 2 // kernel_height // kernel_width
    ...
    grid_coord_im2col = grid_coord_im2col.reshape(1, 1, out_height, out_width, -1, 2) # 1,1, o_h, o_w, k_s^2, 2


    repeat_time = groups//offset_groups
    if repeat_time == 0 or groups % offset_groups != 0:
        raise RuntimeError(
            "The real number of groups should be a multiple of offset_groups.\n"
            f"Got {groups=}, {offset_groups=}"
        )

    offset = offset.view(batch_size, offset_groups, 1, k, 2, out_height, out_width).repeat(1, 1, repeat_time, 1, 1, 1, 1) # Relative offset
    offset = offset.view(batch_size, groups, k, 2, out_height, out_width)
    offset = offset.permute([0, 1, 4, 5, 2, 3])
    coord = (offset + grid_coord_im2col) # Absolute offset
...
class SeparableDeformConv2d(DeformConv2d):
    def forward(self, input: Tensor, offset: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        return separable_deform_conv2d(
            input,
            offset,
            self.weight,
            self.bias,
